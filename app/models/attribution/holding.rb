...
class Attribution::Holding < ActiveRecord::Base
  ...
  def associate_axys_holdings
    ...
    h.update_attribute :emv_holding_id, self.id

    ph = axys_portfolio.holdings.where( date: day.date.prev_trading_day, company_id: company_id ).first
    ph.update_attribute( :bmv_holding_id, self.id ) if ph
  end
  
  def associate_axys_transactions
    date_range = (day.date.prev_trading_day+1..day.date)
  ...
  def secondary_txns
    day.axys_portfolio.transactions.where( sd_symbol: company.tag )
  end
